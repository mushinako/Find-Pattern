...
function divider(n: number): Array<number> {
    if (n === 1) return [];
    let f: Factor = factor(n);
    let count: number = 1;
    let nums: Array<number> = [];
    let occs: Array<number> = [];
...
function mu(n: number): number {
    let f: Array<[string, number]> = Object.entries(factor(n));
    return f.some(cur => cur[1] > 1) ? 0 : f.length % 2 ? -1 : 1;
}

function calc(b: number, p: number): [number, Array<number>] {
    ...
    let sum: number = 0;
    let sums: Array<number> = Array(b).fill(0);
    let dividers: Array<number> = divider(p);
